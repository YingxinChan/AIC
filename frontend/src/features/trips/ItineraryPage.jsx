...
export default function ItineraryPage() {
  ...
  // Click-to-expand activity details (photo/website/walking distance). All
  // three are live-looked-up here, never persisted or generated by Claude —
  // see the ticket for why (Claude's own stored activity.lat/lng were tested
  // against real geocoded coordinates and found off by up to 1.6km for some
  // activities, fine for a map pin, not fine for a distance presented as fact).
  const resolveCoords = (activity, placeDetails) => {
    if (placeDetails?.lat != null && placeDetails?.lon != null) {
      return { lat: placeDetails.lat, lng: placeDetails.lon }
    }
    return { lat: activity.lat, lng: activity.lng } // fallback: Claude's guess, only used if Nominatim found nothing
  }

  const toggleExpand = async (activity, nextActivity) => {
    if (expandedId === activity.id) {
      setExpandedId(null)
      return
    }
    setExpandedId(activity.id)
    if (detailsById[activity.id]) return // already fetched, don't re-fetch

    const [photo, placeDetails, nextPlaceDetails] = await Promise.all([
      getActivityPhoto(activity.name),
      getPlaceDetails(activity.name, destination),
      nextActivity ? getPlaceDetails(nextActivity.name, destination) : null,
    ])

    let walking = null
    if (nextActivity) {
      const from = resolveCoords(activity, placeDetails)
      const to = resolveCoords(nextActivity, nextPlaceDetails)
      walking = await getWalkingDistance(from.lat, from.lng, to.lat, to.lng)
    }

    setDetailsById((prev) => ({
      ...prev,
      [activity.id]: { photo, website: placeDetails?.website ?? null, walking },
    }))
  }
          ...
          <div className="border-t border-gray-100 pt-6">
            ...
            <ul className="space-y-2">
              {itineraryDay.activities.map((activity, index) => {
                const nextActivity = itineraryDay.activities[index + 1]
                const details = detailsById[activity.id]
                return (
                <li
                  key={activity.id}
                  onClick={() => toggleExpand(activity, nextActivity)}
                  className="flex items-start gap-3 bg-gray-50 rounded-lg p-3 cursor-pointer hover:bg-gray-100 transition-colors"
                >
                  ...
                  <div className="flex-1">
                    ...
                    {expandedId === activity.id && (
                      <div className="mt-3 pt-3 border-t border-gray-200 space-y-2" onClick={(e) => e.stopPropagation()}>
                        {details === undefined ? (
                          <p className="text-xs text-gray-400 italic">Loading details...</p>
                        ) : (
                          <>
                            {details.photo && (
                              <img
                            ...
                            {details.walking?.distance_m != null && (
                              <p className="text-xs text-gray-500">
                                {details.walking.duration_min} min walk ({details.walking.distance_m}m) to next activity
                              </p>
                            )}
                          </>
                        )}

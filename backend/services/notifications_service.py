...
async def send_test_email(db: AsyncSession, user_id: int) -> dict:
    ...
    return await asyncio.to_thread(
        email_service.send_email, user.email, "Navia test email", html, text,
    )
...
async def send_swap_digest_emails(
    ...
    for user_id in set(by_user_swaps) | set(by_user_tips) | set(by_user_reverted):
        ...
        subject = (
            "Navia: itinerary updated for weather" if (user_swaps or user_reverted)
            else "Navia: weather tips for your trip"
        )
